...
{
        return ((square(mPosition.x-Global::ship->mPosition.x)+
                 square(mPosition.y-Global::ship->mPosition.y))
                 <= square(600));
}
...
void ScreenObject::move(double dt)
{
        ...
        if (mVelocity.z < 0) PhysicsObject::accelerate(0.0, 0.0, -amt);
        if (mVelocity.z > 0) PhysicsObject::accelerate(0.0, 0.0,  amt);
        if (speed > mMaxSpeed)
                decelerate(dt);
        reset();

...
void Bogey::draw()
{
        ...
        r = 1-strength; g = 0; b = strength / 1.25;
        ...
        if (getState() == ALIVE)
        {
                ...
                if (mDrawBLINE && !mFlock) draw::setColor(.2, .2, .2);
                else draw::setColor(r, g, b, 1.0);
                glCallList(BOGEY);
                glPopMatrix();
        }
        else if (getState() == DYING)
...
void Bogey::fire(double dt)
{
        ...
        double directionToShip =
                atan2((double) (SX(Global::ship)-SX(this)),

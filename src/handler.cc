...
void PlayHandler::process(double dt)
{
	...
	if (mouseMotion())
	{
		Global::ship->rotation =
			atan2((double) (mouseX()-(Screen::mDisplay.x>>1)),
			      (double) ((Screen::mDisplay.y>>1)-mouseY()));
		Global::ship->rotate(0);
	}
	else
	{
		if (keyPressed(SDLK_RIGHT)) Global::ship->rotate(RAD( 5) * dt);
		if (keyPressed(SDLK_LEFT))  Global::ship->rotate(RAD(-5) * dt);
	}
...
void MenuHandler::process(double dt)
{
	...
	if (!canmenu) return;
	else if (mup)	mGameMenu->moveUp();
	else if (mdown) mGameMenu->moveDown();
	else if (mleft) mGameMenu->moveLeft();
	else if (mright) mGameMenu->moveRight();
	else if (mexec) mGameMenu->exec(mController);

...
FlockMember::FlockMember(ObjectType type,
        ...
        : ScreenObject(type, radius, mass, ms, x, y, z, fx, fy, fz),
          mFlock(0), mAvoidance(0,0,0), mAvoidCount(0)
{
        mFlocking = true;
}
...
public:
        ...
        inline
        void accumulate(Coord3<double> &a,
                        Coord3<double> &b,
                        int weight)
        {
                Coord3<double> t;
...
void FlockMember::move(double dt)
{
        ...
        if (mFlock) mFlock->updateCentroid(*this);
...
Flock::Flock()
        : r(((float) (rand() % 100000)) / 100000.0f),
          ...
          mTarget(0,0, 0), mTargetAge((double) (rand() % (int) MAX_TARGET_AGE)),
          mMaxMembers(((rand() % (FLOCK_MAX_MEMBERS-FLOCK_MIN_MEMBERS-1))
                          +FLOCK_MIN_MEMBERS)),
          mNumMembers(0), mMembersUpdated(0)
{
        setTarget();
}
...
void Flock::checkTarget(double dt)
{
        ...
        if ((mTargetAge > MAX_TARGET_AGE) ||
            ((square(mTarget.x-mCenterPos.x) +
              square(mTarget.y-mCenterPos.y)) <= 1000))
                setTarget();
}
...
void Flock::updateCentroid(FlockMember &obj)
{
        ...
        if (mag > 25)   mTmpCenterForce += oForce / mag * 25;
        else            mTmpCenterForce += oForce;

...
const char* configFile()
{
    ...
    if (isfile(HOMEDIR_CONF)) {
        return HOMEDIR_CONF;
    } else if (isfile(PATH2)) {
        return PATH2;
    } else if (isfile(PATH3)) {
        return PATH3;
    } else if (isfile(PATH4)) {
        return PATH4;
    } else if (isfile(PATH5)) {
        return PATH5;
    }
    return 0;
}
...
public:
    ...
    void applyValue()
    {
        if (sectionIs("video")) {
            applyVideo();
        } else if (sectionIs("game")) {
            applyGame();
        } else if (sectionIs("camera")) {
            applyCamera();
        } else if (sectionIs("audio")) {
            applyAudio();
        } else {
            error();
        }
